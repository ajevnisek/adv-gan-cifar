...
def test_model(model):
    cifar_dataset_test = torchvision.datasets.CIFAR10(
    ...
    print('accuracy in testing set: %f\n' % (
            num_correct.item() / len(cifar_dataset_test) * 100.0))
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        loss_epoch = 0
        if epoch == 100:
            opt_model = torch.optim.SGD(target_model.parameters(), lr=0.0001)
        for i, data in enumerate(train_dataloader, 0):
            train_imgs, train_labels = data
            train_imgs, train_labels = train_imgs.to(device), train_labels.to(
                device)
